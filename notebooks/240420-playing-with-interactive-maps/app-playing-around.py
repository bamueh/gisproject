...
def get_info(feature=None):
    ...
    return header + [
        html.B(feature["properties"]["name"]), html.Br(),
        "{:.3f} people / mi".format(feature["properties"]["density"]),
        html.Sup("2")] + [
        html.B(), html.Br(),
        "{} something".format(feature["properties"]["xx"])]

# # Create colorbar.
classes = [0, 10, 20, 50, 100, 200, 500, 1000]
style = dict(weight=2, opacity=1, color='white', dashArray='3', fillOpacity=0.7)
colorscale = ['#FFEDA0', '#FED976', '#FEB24C', '#FD8D3C', '#FC4E2A',
              '#E31A1C', '#BD0026', '#800026']
# js lib used for colors
chroma = "https://cdnjs.cloudflare.com/ajax/libs/chroma-js/2.1.0/chroma.min.js"
colorbar = dl.Colorbar(colorscale=colorscale, width=300, height=30, min=0, max=1000, position='bottomleft') #, unit='/km2')
...
geojson = dl.GeoJSON(url="/assets/us-states.json",  # url to geojson file
                     ...
                     hoverStyle=arrow_function(dict(weight=1, color='#666',
                                                    dashArray='')),
                     hideout=dict(colorscale=colorscale, classes=classes,
                                  style=style, colorProp="density", min=0,
                                  max=1000),
                     id="geojson")
...
app.layout = html.Div([
    # # Slider to pick the dataset to display
    # dcc.Slider(
    #     id="dataset-slider",
    #     min=1,
    #     max=2,
    #     step=1,
    #     marks={1: "US States", 2: "Other Dataset"},
    ...
    dl.Map(children=[
        ...
        colorbar,
        info
    ], style={'height': '50vh'}, center=[40.52695, -97.59588], zoom=4),
    # Input component for user to specify density threshold
    html.Label("Density Threshold"),
    # Slider for density threshold
    dcc.Slider(id="density-threshold", min=0, max=max(classes), step=1,
               value=0,
...
def update_geojson(value):
    filtered_features = [
        feature for feature in us_states["features"] if
        feature["properties"]["density"] > value]
    return {"type": "FeatureCollection", "features": filtered_features}

# @app.callback(
#     Output("geojson", "data"),
#     [Input("density-threshold", "value"),
#      Input("dataset-slider", "value")]
# )
# def update_geojson(density_threshold, selected_dataset):
#     if dash.callback_context.triggered[0]["prop_id"] == "density-threshold.value":
#         # Update GeoJSON based on density threshold
#         filtered_features = [
#             feature for feature in us_states["features"] if
#             feature["properties"]["density"] > density_threshold]
#         return {"type": "FeatureCollection", "features": filtered_features}
#     elif dash.callback_context.triggered[0]["prop_id"] == "dataset-slider.value":
#         # Update GeoJSON based on selected dataset
...
# @app.callback(Output("info", "children"), Input("geojson", "hoverData"))
# def info_hover(hover_data):
#     if hover_data:
#         feature = hover_data["properties"]
#         return get_info(feature)
#     else:
#         return get_info()
...
#     return us_states
#     # if selected_dataset == "us-states":
#     #     return us_states
#     # elif selected_dataset == "other-dataset":
#     #     return us_states_2
